...
class GraphData(
    ...
    fun addBgReadings(addPredictions: Boolean, context: Context?) {
        // ⬇️ USAR O VALOR MÍNIMO CALCULADO (se disponível)
        minY = if (overviewData.minBgValue != Double.MAX_VALUE) {
            overviewData.minBgValue
        } else {
            // Fallback: usar low line como mínimo
            defaultValueHelper.determineLowLine()
        }
        ...
        // ⬇️ GARANTIR QUE maxY SEJA MAIOR QUE minY
        if (maxY <= minY) {
            maxY = minY + if (units == GlucoseUnit.MGDL) 40.0 else 2.2
        }
